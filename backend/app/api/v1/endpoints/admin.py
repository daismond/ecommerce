...
@router.post("/coupons/", response_model=schemas.coupon.Coupon, status_code=status.HTTP_201_CREATED)
def create_coupon(
    coupon: schemas.coupon.CouponCreate,
    db: Session = Depends(get_db),
    current_user: models.user.User = Depends(get_current_admin_user)
):
    """
    ...
    """
    return crud.coupon.create_coupon(db=db, coupon=coupon)

@router.get("/coupons/", response_model=List[schemas.coupon.Coupon])
def read_coupons(
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db),
    ...
    """
    coupons = crud.coupon.get_coupons(db, skip=skip, limit=limit)
    return coupons

@router.patch("/coupons/{coupon_id}", response_model=schemas.coupon.Coupon)
def update_coupon(
    coupon_id: uuid.UUID,
    coupon_update: schemas.coupon.CouponUpdate,
    db: Session = Depends(get_db),
    current_user: models.user.User = Depends(get_current_admin_user)
):
    """
    ...
    """
    updated_coupon = crud.coupon.update_coupon(db, coupon_id=coupon_id, coupon_update=coupon_update)
    if updated_coupon is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Coupon not found")
    return updated_coupon

@router.delete("/coupons/{coupon_id}", response_model=schemas.coupon.Coupon)
def delete_coupon(
    coupon_id: uuid.UUID,
    db: Session = Depends(get_db),
    current_user: models.user.User = Depends(get_current_admin_user)
    ...
    """
    deleted_coupon = crud.coupon.delete_coupon(db, coupon_id=coupon_id)
