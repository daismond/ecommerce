...
@router.post("/apply-coupon", response_model=schemas.cart.Cart)
def apply_coupon(
    coupon_data: schemas.cart.ApplyCoupon,
    cart: models.cart.Cart = Depends(get_or_create_cart),
    db: Session = Depends(get_db)
):
    """
    ...
    """
    updated_cart = crud.cart.apply_coupon_to_cart(db, cart=cart, coupon_code=coupon_data.coupon_code)
    if not updated_cart:
